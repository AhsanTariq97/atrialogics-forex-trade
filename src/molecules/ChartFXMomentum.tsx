...
const ChartFXMomentum: React.FC<Props> = ({ fxData, setfxData }) => {

    const [ NPCI, setNPCI ] = useState<number>(fxData.momentum.NPCI)
    const [ color, setColor ] = useState<number>(fxData.momentum.color)

    const [momentum, setMomentum] = useState({NPCI: NPCI, color: color})

    const setMomentumFn = () => {
        setMomentum({...momentum, NPCI: NPCI, color: color})
    }
    
    useEffect(() => {
        setfxData({...fxData, momentum})
    }, [momentum])
    ...
    <div className='flex flex-col items-start justify-between w-full h-full px-2 py-4 space-y-8'>
        <div className='flex flex-col items-start justify-between w-full text-center'>
            ...
            <div className='grid items-center justify-center w-full grid-cols-7'>
                ...
                <input type="number" className='text-right bg-transparent outline-none border border-[#CDCDCD] rounded' min={1} max={99} step={1} defaultValue={fxData.momentum.color || 1} onChange={(e) => setColor(parseInt(e.target.value))} />
                <span>px</span>
                {/* <p className='col-span-6 py-1 pl-4 text-left border border-[#CDCDCD] rounded'>1 px</p> */}
            </div>
        </div>
        <button className='w-[208px] bg-[#5290F7] text-sm font-semibold rounded text-white py-4 self-end' onClick={setMomentumFn}>ADD MOMENTUM</button>
    </div>
  )
}

export default ChartFXMomentum